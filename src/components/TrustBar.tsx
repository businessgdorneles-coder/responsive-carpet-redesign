        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {items.map((item) => (
            <div key={item.title} className="flex items-center gap-3">
              <img src={item.icon} alt={item.title} className="w-10 h-10 object-contain shrink-0 hover-scale" style={{ filter: "invert(39%) sepia(98%) saturate(1500%) hue-rotate(202deg) brightness(97%) contrast(101%)" }} />
              <div>
